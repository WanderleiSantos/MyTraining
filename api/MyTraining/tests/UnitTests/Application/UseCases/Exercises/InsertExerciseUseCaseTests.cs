...
public class InsertExerciseUseCaseTests
{
    ...
    public async Task ShouldReturnErrorValidationIfCommandIsEmpty()
    {
        ...
        output.Errors.Should().Contain(e => e.Code!.Contains("UserId")).Which.Description.Should()
            .Be("'User Id' must not be empty.");
        output.Errors.Should().Contain(e => e.Code!.Contains("Name")).Which.Description.Should()
